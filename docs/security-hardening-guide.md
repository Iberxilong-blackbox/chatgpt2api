...
| # | 措施 | 状态 |
|---|------|------|
| 1 | SSH 禁用密码登录，只用密钥 | ✅ 已完成 |
| 2 | SSH 端口已更改 | ✅ 已完成 |
| 3 | 防火墙只开放 80/443/SSH | ✅ 已完成 |
| 4 | Nginx 拒绝 `.env`、`.git`、`/data` 访问 | ✅ 已完成 |
| 5 | Nginx 加了安全响应头 | ✅ 已完成 |
| 6 | 管理面板 / 登录接口 IP 白名单生效 | ✅ 已完成 |
| 7 | fail2ban 运行中且检测登录失败 | ✅ 已完成 |
| 8 | 注册已关闭（如不需要） | ✅ 已完成 |
| 9 | Go 服务只监听 127.0.0.1 | ✅ 已完成 |
| 10 | HTTPS 已配置（如使用域名） | ✅ 已完成 |
| 11 | curl 敏感路径均返回 404 | ✅ 已完成 |
